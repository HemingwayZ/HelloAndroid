...
import com.hemingway.editpic.clip.IMGClip;
import com.hemingway.editpic.clip.IMGClipWindow;
import com.hemingway.editpic.homing.IMGHoming;
import com.hemingway.editpic.sticker.IMGSticker;
import com.hemingway.editpic.util.IMGUtils;
...
public class IMGImage implements Serializable {
    ...
    private int picDetail;
    private int picBright;
    private int picContrast;
    /**
     * 完整图片边框
     */
     ...
     * 涂鸦路径 重做
     */
    private List<IMGPath> mDoodlesRedo = new ArrayList<>();
    /**
     * 马赛克路径
     */
    ...
    private static final int COLOR_SHADE = 0x64000000;
    ...
    public void undoDoodle() {
        if (!mDoodles.isEmpty()) {
            IMGPath remove = mDoodles.remove(mDoodles.size() - 1);
            mDoodlesRedo.add(remove);
        }
    }

    public void redoDoodle(){
        if(!mDoodlesRedo.isEmpty()){
            IMGPath remove = mDoodlesRedo.remove(mDoodlesRedo.size() - 1);
            mDoodles.add(remove);
        }
    }
...
//                    mClipWin
    ...
    public void setmClipWin(IMGClipWindow mClipWin) {
        this.mClipWin = mClipWin;
    }
    ...
    public void onScale(float factor, float focusX, float focusY) {
        ...
        if (!mFrame.contains(mClipFrame)) {
            ...
            if(DEBUG){
                Log.d(TAG,"hemingway contains-->"+factor);
            }
        }
    ...
    public boolean canUndo() {
        return mDoodles != null && mDoodles.size() > 0;
    }
    public boolean canRedo() {
        return mDoodlesRedo != null && mDoodlesRedo.size() > 0;
    }

    public int getPicDetail() {
        return picDetail;
    }

    public void setPicDetail(int picDetail) {
    ...
    public void setPicBright(int picBright) {
        this.picBright = picBright;
    }
